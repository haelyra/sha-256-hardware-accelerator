//This testbench was inspired by an ECE111 Lecture at UCSD
module testbench_bitcoin_hash();
...
logic [31:0] dpsram_tb[0:16383];
logic [31:0] message_seed = 32'h01234567;
...
begin
 ...
 $display("Header, 19 words:");

 for (m = 0; m < 19; m++) begin
 if (m == 0)
 ...
 $display("Hash result comparison:");

 num_errors = 0;
 for (n = 0; n < NUM_NONCES; n++) begin
 if (h0[n] !== dpsram[output_addr+n]) begin
 $display("Correct H0[%2d] = %x\tOutput H0[%2d] = %x\tERROR", n, h0[n], n, dpsram[output_addr+n]);
 num_errors = num_errors + 1;
 end else begin
 $display("Correct H0[%2d] = %x\tOutput H0[%2d] = %x", n, h0[n], n, dpsram[output_addr+n]);
 end
 end
  $display("__________________________\n");
 ...
 if (num_errors === 0)
 $display("Hash results as expected\n");
 else
 $display("Error. %2d hash results are incorrect.\n", num_errors);
